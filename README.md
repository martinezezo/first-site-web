# first-site-web

---

https://martinezezo.github.io/first-site-web/

---

WEB DEV



---



WebDev/Body analogy:
- https://miro.medium.com/max/564/1*KRq7jQOUdQsHNsZa72XcDw.jpeg



---



What is HTML?

Created in 1989 by Tim Berners-Lee (CERN) (http, html, url).

- HTML stands for Hyper Text Markup Language
- HTML is the standard markup language for creating Web pages
- HTML describes the structure of a Web page
- HTML consists of a series of elements
- HTML elements tell the browser how to display the content
- HTML elements label pieces of content such as "this is a heading", "this is a paragraph", "this is a link", etc.


Website example using HTML:
- http://info.cern.ch/
- http://info.cern.ch/hypertext/WWW/TheProject.html
- https://www.w3.org/People/Berners-Lee/
- https://de.wikipedia.org/wiki/Hypertext_Markup_Language


DOCUMENTATION - W3SCHOOLS:
- https://www.w3schools.com/html/html_intro.asp

DOCUMENTATION - MDN Web Docs (Mozilla Developer Network):
- https://developer.mozilla.org/en-US/docs/Web/HTML


HTML HISTORY:
- https://www.w3.org/People/Raggett/book4/ch02.html
- https://www.bu.edu/lernet/artemis/years/2020/projects/FinalPresentations/HTML/historyofhtml.html
- https://youtu.be/41mnNyMxPOA



---



What is CSS?

Created in 1994 by Håkon Wium Lie (CERN).

- CSS stands for Cascading Style Sheets
- CSS describes how HTML elements are to be displayed on screen, paper, or in other media
- CSS saves a lot of work. It can control the layout of multiple web pages all at once
- External stylesheets are stored in CSS files


Website example using HTML and CSS:
- https://www.haribo.com/de-de


DOCUMENTATION - W3SCHOOLS:
- https://www.w3schools.com/css/css_intro.asp

DOCUMENTATION - MDN Web Docs (Mozilla Developer Network):
- https://developer.mozilla.org/en-US/docs/Web/CSS


CSS HISTORY:
- https://www.w3.org/Style/CSS20/history.html
- https://www.bu.edu/lernet/artemis/years/2020/projects/FinalPresentations/HTML/historyofcss.html
- https://youtu.be/LeC26IO1WV8



---



What is JavaScript?

Created in 1995 by Brendan Eich (NETSCAPE).

- JavaScript is the Programming Language for the Web.
- JavaScript can update and change both HTML and CSS.
- JavaScript can calculate, manipulate and validate data.


Website example using HTML, CSS and JavaScript:
- https://www.amazon.de/


DOCUMENTATION - W3SCHOOLS:
- https://www.w3schools.com/js/

DOCUMENTATION - MDN Web Docs (Mozilla Developer Network):
- https://developer.mozilla.org/en-US/docs/Web/JavaScript


JS HISTORY:
- https://www.w3schools.com/js/js_history.asp
- https://www.bu.edu/lernet/artemis/years/2020/projects/FinalPresentations/HTML/addingjava.html
- https://youtu.be/3-9fnjzmXWA
